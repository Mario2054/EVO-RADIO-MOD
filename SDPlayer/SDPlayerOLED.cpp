...
void SDPlayerOLED::drawTopBar() {
    ...
    if (_infoStyle == INFO_CLOCK_DATE) {
        // **ZEGAR PO LEWEJ** + **DATA W ŚRODKU** + **GŁOŚNIK + VOLUME PO PRAWEJ**
        struct tm timeinfo;
        if (getLocalTime(&timeinfo, 100)) {
            ...
            int speakerX = 256 - volWidth - 18;  // 18px = szerokość ikony + margines
            int speakerY = 3;
            drawVolumeIcon(speakerX, speakerY);
            
            int volX = speakerX + 12;  // Zaraz po ikonie głośnika
...
void SDPlayerOLED::drawVolumeIcon(int x, int y) {
    // Prosty głośnik
    _display.drawPixel(x, y+3);
    _display.drawPixel(x+1, y+2);
    _display.drawPixel(x+1, y+4);
    _display.drawBox(x+2, y+1, 2, 5);
    _display.drawPixel(x+4, y);
    _display.drawPixel(x+4, y+6);
    _display.drawPixel(x+5, y);
    _display.drawPixel(x+5, y+6);
}
